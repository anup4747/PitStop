-- Pitstop Solutions commerce schema
...
create table public.coupons (
  ...
  is_active boolean not null default true,
  constraint one_discount_type check ((discount_percent is not null) <> (discount_amount is not null))
);
...
for select using (exists (select 1 from public.products p where p.id = product_id and p.is_active));
...
create policy "Users manage their addresses" on public.addresses
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "Users manage their carts" on public.carts
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
for select using (exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid()));
create policy "Users manage their reviews" on public.product_reviews
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "Users manage their wishlist" on public.wishlists
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
